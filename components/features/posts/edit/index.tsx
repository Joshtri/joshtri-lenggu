...
import { UpdatePostInput, Post } from "../interfaces/posts";
import { useUpdatePost, usePost } from "@/services/postsService";
...
export default function PostEdit({ postId }: PostEditProps) {
  ...
  // Auto-generate slug from title (only if title changed from original)
  useEffect(() => {
    if (watchedTitle && post && watchedTitle !== post.title) {
  ...
  const onSubmit = async (data: PostFormData) => {
    try {
      ...
      // Validate coverImageUrl before updating post
  ...
  return (
    <div className="min-h-screen">
      <PageHeader
        title="Edit Post"
        description="Update your blog post content and metadata."
        actions={
          <>
            {isDirty && (
            ...
            <Button
              ...
              onPress={() => router.push(`/posts/${postId}`)}
            >
              Cancel
            </Button>
            <Button
